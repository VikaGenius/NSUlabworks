...
Storage* init(int count) {
    ...
    pthread_rwlock_init(&list->sync_list, NULL);
...
void random_nodes_swap(Storage* list) {
    ...
    pthread_rwlock_wrlock(&list->sync_list);
    pthread_rwlock_wrlock(&list->head->sync);
    Node* first = list->head;
    
    Node* second = NULL;

    if (first->next && rand() % 2 == 1) {
        pthread_rwlock_wrlock(&first->next->sync);
        second = first->next;
        if (second->next) {
            pthread_rwlock_wrlock(&second->next->sync);
            first->next = second->next;
            pthread_rwlock_unlock(&second->next->sync);
        } else {
        ...
        list->head = second;
        pthread_rwlock_unlock(&list->sync_list);
    ...
    while (first->next) {
        ...
        if (second->next) {
            ...
            if (third != NULL && rand() % 2 == 1) { 
                ...
                if (third->next) {
                    pthread_rwlock_wrlock(&third->next->sync);
                    second->next = third->next;
                    pthread_rwlock_unlock(&third->next->sync);
                } else {
...
void count_good_pair(Storage* list, int (*cmp)(size_t, size_t)) {
    ...
    pthread_rwlock_rdlock(&list->sync_list);
    pthread_rwlock_rdlock(&list->head->sync);
