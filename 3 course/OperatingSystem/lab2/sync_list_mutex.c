...
Storage* init(int count) {
    ...
    pthread_mutex_init(&list->sync_list, NULL);
...
void random_nodes_swap(Storage* list) {
    ...
    pthread_mutex_lock(&list->sync_list);
    ...
    if (first->next && rand() % 2 == 1) {
        ...
        if (second->next) {
            ...
            pthread_mutex_unlock(&second->next->sync);
        } else {
        ...
        list->head = second;
        pthread_mutex_unlock(&list->sync_list);
